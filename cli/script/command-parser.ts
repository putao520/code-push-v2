...
import {IFreeCommand} from "../definitions/cli";
...
function groupList(commandName: string, yargs: yargs.Argv): void {
    isValidCommand = true;
    yargs.usage(USAGE_PREFIX + " group " + commandName + " [options]")
        .demand(/*count*/ 0, /*max*/ 0)
        .example("group " + commandName, "List your groups in tabular format")
        .example("group " + commandName + " --format json", "List your groups in JSON format")
        .option("format", { default: "table", demand: false, description: "Output format to display your group with (\"json\" or \"table\")", type: "string" });

    addCommonConfiguration(yargs);
}

function groupRemove(commandName: string, yargs: yargs.Argv): void {
    isValidCommand = true;
    yargs.usage(USAGE_PREFIX + " group " + commandName + " <groupName>")
        .demand(/*count*/ 1, /*max*/ 1)  // Require exactly one non-option arguments
        .example("group " + commandName + " MyGroup", "Removes group \"MyGroup\"");

    addCommonConfiguration(yargs);
}

function groupAttach(commandName: string, yargs: yargs.Argv): void {
    isValidCommand = true;
    yargs.usage(USAGE_PREFIX + " group " + commandName + " <groupName> <deploymentKey>")
        .demand(/*count*/ 2, /*max*/ 2)  // Require exactly one non-option arguments
        .example("group " + commandName + " MyGroup" + "Your_DeploymentKey", "attach group \"MyGroup\" to your a DeploymentKey");

    addCommonConfiguration(yargs);
}

function groupDetach(commandName: string, yargs: yargs.Argv): void {
    isValidCommand = true;
    yargs.usage(USAGE_PREFIX + " group " + commandName + " <groupName> <deploymentKey>")
        .demand(/*count*/ 2, /*max*/ 2)  // Require exactly one non-option arguments
        .example("group " + commandName + " MyGroup" + "Your_DeploymentKey", "detach group \"MyGroup\" to your a DeploymentKey");
...
var argv = yargs.usage(USAGE_PREFIX + " <command>")
    ...
    .command("app", "View and manage your CodePush apps", (yargs: yargs.Argv) => {
        ...
        yargs.usage(USAGE_PREFIX + " app <command>")
            ...
            .command("add", "Add a new app to your account", (yargs: yargs.Argv): void => {
                isValidCommand = true;
                yargs.usage(USAGE_PREFIX + " app add <appName> <os> <platform> <script>")
                    ...
                    .example("app add MyApp android cordova \"f:\\fake_folder\\script.js\"", "Adds app \"MyApp\", indicating that it's an Android Cordova app");
    ...
    .command("patch", "Update the metadata for an existing release", (yargs: yargs.Argv) => {
        yargs.usage(USAGE_PREFIX + " patch <appName> <deploymentName> [options]")
            ...
            .option("script", { alias: "s", default: null, demand: false, description: "app update rule script path.", type: "string" })
    ...
    .command("release-react", "Release a React Native update to an app deployment", (yargs: yargs.Argv) => {
        yargs.usage(USAGE_PREFIX + " release-react <appName> <platform> [options]")
            ...
            .option("region", { alias: "n", default: null, demand: false, description: "Package region name", type: "string" })
            .check((argv: any, aliases: { [aliases: string]: string }): any => { return checkValidReleaseOptions(argv); });
    ...
    .command("rollback", "Rollback the latest release for an app deployment", (yargs: yargs.Argv) => {
        yargs.usage(USAGE_PREFIX + " rollback <appName> <deploymentName> [options]")
            ...
            .option("regionName", { alias: "n", default: null, demand: false, description: "Package region name", type: "string" });
    ...
    */
    .command("free", "free release", (yargs: yargs.Argv) => {
        isValidCommandCategory = true;
        isValidCommand = true;
        yargs.usage(USAGE_PREFIX + " free <appName> <deploymentName> <region>")
            .demand(/*count*/ 2, /*max*/ 3)  // Require exactly two non-option arguments
            .example("free MyApp Production", "free the \"Production\" deployment of \"MyApp\"")
            .example("free MyApp Production RegionName", "free the \"Production\" deployment of \"MyApp\" in the \"RegionName\" region")
        addCommonConfiguration(yargs);
    })
        ...
        switch (arg0) {
            ...
            case "patch":
                if (arg1 && arg2) {
                    ...
                    patchCommand.script = argv["script"];
                }
                break;
            ...
            case "release-react":
                if (arg1 && arg2) {
                    ...
                    releaseReactCommand.region = argv["region"];
                }
                break;
            ...
            case "rollback":
                if (arg1 && arg2) {
                    ...
                    rollbackCommand.regionName = argv["regionName"];
                }
                break;
            ...
            case "free":
                if (arg1 && arg2) {
                    cmd = {type: cli.CommandType.free};
                    var IFreeCommand = <cli.IFreeCommand>cmd;

                    IFreeCommand.appName = arg1;
                    IFreeCommand.deploymentName = arg2;
                    IFreeCommand.regionName = arg2
                }
                break

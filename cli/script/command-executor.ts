...
import AccountManager = require("code-push-v2");
...
import {
    ...
    Session,
    UpdateMetrics
} from "code-push-v2/script/types";
import {Group} from "rest-definitions";
...
function appAdd(command: cli.IAppAddCommand): Promise<void> {
    ...
    const scriptBase64 = flushScript2base64(command.script);

    return sdk.addApp(command.appName, os, platform, scriptBase64, false)
...
function printGroupList(format: string, groups: Group[]): void {
    if (format === "json") {
        printJson(groups);
    } else if (format === "table") {
        const headers = ["Name", "Description"];
        printTable(headers, (dataSource: any[]): void => {
            groups.forEach((group: Group, index: number): void => {
                const row = [group.name, group.description];
                dataSource.push(row);
            });
        });
    }
...
function patch(command: cli.IPatchCommand): Promise<void> {
    ...
    for (var updateProperty in packageInfo) {
        if ((<any>packageInfo)[updateProperty] !== null) {
            return sdk.patchRelease(command.appName, command.deploymentName, command.label, scriptBase64, packageInfo)
                .then((): void => {
...
function flushScript2base64(scriptPath: string): string {
    if( !fs.existsSync(scriptPath) ){
        return ""
    }
    return fs.readFileSync(scriptPath).toString('base64')
}

export var releaseCordova = (command: cli.IReleaseCordovaCommand): Promise<void> => {
    var releaseCommand: cli.IReleaseCommand = <any>command;
    var region: string = command.region || "";
    // Check for app and deployment exist before releasing an update.
    // This validation helps to save about 1 minute or more in case user has typed wrong app or deployment name.
    return validateDeployment(command.appName, command.deploymentName, region)
        .then((): any => {
...
function validateDeployment(appName: string, deploymentName: string, region: string): Promise<void> {
    return sdk.getDeployment(appName, deploymentName, region)
        .catch((err: any) => {
...
function rollback(command: cli.IRollbackCommand): Promise<void> {
    return confirm()
        .then((wasConfirmed: boolean) => {
            ...
            return sdk.rollback(command.appName, command.deploymentName, command.targetRelease || undefined, command.regionName || undefined)
                .then((): void => {
...
function free(command: cli.IFreeCommand): Promise<void> {
    return confirm()
        .then((wasConfirmed: boolean) => {
            if (!wasConfirmed) {
                log("Free release cancelled.")
                return;
            }
            return sdk.freeRelease(command.appName, command.deploymentName, command.regionName || undefined)
                .then((): void => {
                    log("Successfully performed a free on the \"" + command.deploymentName + "\" deployment of the \"" + command.appName + "\" app in the \"" + command.regionName + "\".");
                });
        });
}

...
class AccountManager {
    ...
    public addApp(appName: string, appOs: string, appPlatform: string, scriptBase64: string, manuallyProvisionDeployments: boolean = false): Promise<App> {
    ...
    */
    ...
    public getDeployment(appName: string, deploymentName: string, region: string): Promise<Deployment> {
        const url = (region && region !== "") ?
            urlEncode `/apps/${this.appNameParam(appName)}/deployments/${deploymentName}/region/${region}` :
            urlEncode `/apps/${this.appNameParam(appName)}/deployments/${deploymentName}`
        return this.get(url)
            .then((res: JsonResponse) => res.body.deployment);
    }
    ...
    public release(appName: string, deploymentName: string, filePath: string, targetBinaryVersion: string, regionName: string, updateMetadata: PackageInfo, uploadProgressCallback?: (progress: number) => void): Promise<Package> {
    ...
    public patchRelease(appName: string, deploymentName: string, label: string, scriptBase64: string, updateMetadata: PackageInfo): Promise<void> {
        updateMetadata.label = label;
        var requestBody: string = JSON.stringify({ packageInfo: updateMetadata, script: scriptBase64 });
    ...
    public rollback(appName: string, deploymentName: string, targetRelease?: string, regionName?: string): Promise<void> {
        return this.post(urlEncode `/apps/${this.appNameParam(appName)}/deployments/${deploymentName}/rollback/${targetRelease || ``}/${regionName || ''}`, /*requestBody=*/ null, /*expectResponseBody=*/ false)
            .then(() => null);
    }

    public freeRelease(appName: string, deploymentName: string, regionName?: string): Promise<void> {
        return this.del(urlEncode `/apps/${this.appNameParam(appName)}/deployments/${deploymentName}/free/${regionName || ''}`)
            .then(() => null);
    }
    ...
    private groupNameParam(appName: string) {
        return appName.replace("/", "~~");
    }
}

...
import {Package, PackageInfo} from "code-push-v2/script/types";
        ...
        .then((packagePath: string): Promise<cli.IReleaseCommand> => {
            ...
            return sdk.isAuthenticated(true)
                .then((isAuth: boolean): Promise<Package> => {
                    const regionName = currentCommand.region || ""
                    return sdk.release(currentCommand.appName, currentCommand.deploymentName, packagePath, currentCommand.appStoreVersion, regionName, updateMetadata, uploadProgress);
                })
                .then((): void => {
